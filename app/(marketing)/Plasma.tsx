...
const hexToRgb = (hex: string): [number, number, number] => {
  ...
  if (!result) return [0.145, 0.388, 0.922]; // Project accent blue (#2563eb)
  return [
...
export const Plasma: React.FC<PlasmaProps> = ({
  ...
  useEffect(() => {
    ...
    const invalidateCachedRect = () => {
      cachedRect = null;
    };

    const handleMouseMove = (e: MouseEvent) => {
      if (!mouseInteractive) return;
      if (!cachedRect && containerEl) {
  ...
  return (
    <div
      ref={containerRef}
      className="w-full h-full relative overflow-hidden contain-strict"
    />
  );
};
